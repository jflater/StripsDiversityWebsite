...
rarefaction_curve_data <- calculate_rarefaction_curves(inc.physeq, c('Observed'), rep(c(1, 10, 100, 1000, 1:100 * 1000), each = 10))
#####
...
sample_data <- data.frame(sample_data(inc.physeq)) 
row.names(sample_data) <- make.names(row.names(sample_data)) # Remove the "-" and replace with ".", to match 'Sample'
row.names(sample_data)
rarefaction_curve_data_summary_verbose <- merge(rarefaction_curve_data_summary, sample_data, by.x = 'Sample', by.y = 'row.names')
library('ggplot2')
...
png("Figures/Wd1RareCurveMinTax1.png",height=5,width=8,units='in',res=600)
plot(plot)
dev.off()
